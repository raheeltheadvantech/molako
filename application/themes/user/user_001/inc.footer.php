        ...
        $.ajax({
            ...
            success: function(json) {
                if (json['success']) {
                    ...
                    $('#price_html').html(json['price_html']);
                    if(json['variant_qty'] <= 5)
                    {
                        $('#qty5').css('display','inline-flex');
                    }
                    else
                    {
                        $('#qty5').hide();
	...
	function place_order()
	{
        ...
        if(document.getElementById('check-agree').checked) {
            ...
            return 0;
                    ...
                    {
                        btn.attr("disabled", false);
                toastr.error('error','Please Select Payment Method');
...
?>
    <script type="text/javascript">
        function ch_img(img)
        {
            var url = '<?= live_img_url().'images/products/medium' ?>/'+img;
