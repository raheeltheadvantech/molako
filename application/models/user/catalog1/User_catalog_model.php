...
class User_catalog_model extends User_Model
{
	...
	function get_products($data=array(), $return_count=false, $is_deals=null, $brand_slug=0)
	{
		...
		else
		{
			...
			if($brand_slug){
				$this->db->where('p.brand_name', $brand_slug);
			}
...
// echo '<pre>';print_r($product); die();
	...
	public function remove_from_wishlist($wish_list_id)
	{
		if($this->db->where('user_wishlist_id', $wish_list_id)->delete('customer_wishlist')){
			return true;
		}else{
			return false;
		}
