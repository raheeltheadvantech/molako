...
//dd($product_id);
	...
	$rel = $this->db->where('product_id',$product_id)->get('related_products')->result();
		foreach($rel as $k => $v)
		{
			$rid[] = $v->related_product_id;
		}
if(!$rid)
{
return array();
		...
		$this->db->where('p.is_enabled', 1);
		$this->db->where_in('p.product_id', $rid);
...
$this->db->like('p.product_name', $search_term);
        ...
        $this->db->or_like('p.long_description', $search_term);

        $this->db->or_like('p.meta_title', $search_term);
 
        $this->db->or_like('p.meta_keywords', $search_term);
$this->db->group_end();
$this->db->where('p.is_enabled', 1);
