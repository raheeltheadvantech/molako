...
class Admin_routes_model extends Admin_Model {
	...
	function save($route)
	{
		if($route['id'])
		{
			$is_exist = $this->db->get_where('routes', array('id'=>$route['id']))->row();
			if($is_exist)
			{
	...
	function validate_route_id($id)
	{
		$this->db->where('id', $id);
		
		$result = $this->db->get('routes')->row();
		if(!$result)
		{
			return false;
		}
	...
	function check_slug($slug, $id=false)
	{
		...
		$result = $this->db->count_all_results('routes');
