...
class Admin_Session {
	...
	{
		log_message('debug', "Admin Session Class Initialized");
        die('sss123');
		...
		log_message('debug', "Admin Session routines successfully run");
	}
	...
	function sess_read()
	{
		...
		if ($session === FALSE)
		{
			log_message('debug', 'A admin session cookie was not found.');
			return FALSE;
		}
		...
		else
		{
			...
			{
				log_message('error', 'The admin session cookie data did not match what was expected. This could be a possible hacking attempt.');
				$this->sess_destroy();
				return FALSE;
			}
		}
	 ...
	 * @return	void
	 */
	function sess_write($is_write_cookie = false)
	{
	 ...
	 * @return	void
	 */
	public function __set($newdata = array(), $newval = '')
	{
		if (is_string($newdata))
		{
			$newdata = array($newdata => $newval);
...
/* Location: ./application/libraries/Admin_Session.php */