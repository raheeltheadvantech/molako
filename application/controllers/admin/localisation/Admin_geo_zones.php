...
class Admin_geo_zones extends Admin_Controller {
    ...
    function index()
    {
        ...
        $data['page_title']	= 'Geo Zones';
        $data['page_header']= 'Geo Zones';

        $params = array();

        $order 		= $this->input->get('order') ? $this->input->get('order') : 'g.geo_zone_id';
        ...
        if($post)
        {
            ...
            redirect(site_url($this->admin_folder .'/'. $this->controller_dir .'/geo_zones.html?code='.$code));
        }
        elseif ($code)
        {
        ...
        $result	= $this->Admin_geo_zone_model->get_geo_zones(array('params'=>$params, 'term'=>$term, 'order'=>$order, 'sort'=>$sort, 'rows'=>$rows, 'per_page'=>$per_page));
        $total	= $this->Admin_geo_zone_model->get_geo_zones(array('params'=>$params, 'term'=>$term, 'order'=>$order, 'sort'=>$sort), true);

        $data['result']	= $result;
        $data['total']	= $total;

        $config['base_url']	= site_url($this->admin_folder .'/'. $this->controller_dir .'/geo_zones.html?order='.$order.'&sort='.$sort.'&code='.$code);
        ...
        $this->view($this->admin_view  .'/'. $this->view_dir .'/geo_zones_list', $data);
    }

    function default_data(&$data)
    {
        $data['page_title']		= 'Geo Zone Form';
        $data['page_header']	= 'Geo Zone Form';
    ...
    private function form($mode = '')
    {
        ...
        if ($mode == 'edit')
        {
            $result	= $this->Admin_geo_zone_model->get_geo_zone($data['id']);
            if(!$result)
            {
                $this->admin_session->set_flashdata('error', 'Geo Zone not found.');
                redirect($this->admin_url .'/'. $this->controller_dir .'/geo_zones.html');
            }
            ...
            $data['zone_to_geo_zone']	= $this->Admin_geo_zone_model->get_zone_to_geo_zone($data['id']);
            ...
            $data['default_country'] = $default_country_id->country_id;




           // $data['route'] = 'tax-edit.html?id='.$data['id'];
            $data['route'] = 'geo_zone_edit.html?id='.$data['id'];
        }
        else{
           ...
           // echo '<pre>';print_r($countries_options);die();
        }

        $this->form_validation->set_rules('name', 'Name', 'trim|required|callback_is_geo_zone_name_already_exist');
        $this->form_validation->set_rules('description', 'Description', 'trim|required');

        if ($this->form_validation->run() == FALSE)
        {
            $this->view($this->admin_view  .'/'. $this->view_dir .'/geo_zones_form', $data);
        }
        else
        {
            $save['geo_zone_id']		= $data['id'];
            $save['name']			= $this->input->post('name');
            $save['description']			= $this->input->post('description');
            $save['zone_to_geo_zone']			= $this->input->post('zone_to_geo_zone');
            ...
            $this->Admin_geo_zone_model->save_geo_zone($save);

            $this->admin_session->set_flashdata('message', 'Geo Zone has been saved.');
            redirect($this->admin_url .'/'. $this->controller_dir .'/geo_zones.html');
        }
    }
	...
	function geo_zone_delete($id = false)
	{


	    $geo_zone_id	= $this->input->get('id');

        $getTaxRate = $this->Admin_geo_zone_model->getTaxRatesByGeoZoneId($geo_zone_id);

	    if($getTaxRate){
            $this->admin_session->set_flashdata('error', 'This geo zone cannot be deleted as it is currently assigned to one or more tax rates!');
            redirect($this->admin_url . '/' . $this->controller_dir . '/geo_zones.html');
        }else {
            $this->Admin_geo_zone_model->delete_zone($geo_zone_id);
            //$this->Admin_geo_zone_model->delete($id);

            $this->admin_session->set_flashdata('message', 'Geo Zone has been deleted Successfully');

            redirect($this->admin_url . '/' . $this->controller_dir . '/geo_zones.html');
        }
	}

    function is_geo_zone_name_already_exist($str)
    {
        ...
        $geo_zone_id	= $this->input->get('id');
        $result = $this->Admin_geo_zone_model->is_geo_zone_name_already_exist($str , $geo_zone_id);
        if ($result)
        {
            $this->form_validation->set_message('is_geo_zone_name_already_exist', 'Geo Zone Name already exist');
            return FALSE;
        }
        else
        {
