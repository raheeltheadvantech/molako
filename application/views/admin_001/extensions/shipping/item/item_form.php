...
<div class="row">
    ...
    <div class="col-md-12">
        ...
        <div class="row">
            <div class="col-md-4">
                <div class="form-group">
                    <label class="required">Tax Classes</label>
                    <?php
                    $options = array();
                    foreach ($tax_classes as $val){
                        $options[$val->tax_class_id] = $val->title;
                    }
                    echo form_dropdown($tax_class_id_name, $options, set_value($tax_class_id_name, $tax_class_id), 'class="form-control" id="taxclasses"');
                    ?>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-4">
                <div class="form-group">
                    <label class="required">Geo Zones</label>
                    <?php
                    $options = array();
                    foreach ($geo_zones as $val){
                        $options[$val->geo_zone_id] = $val->name;
                    }
                    echo form_dropdown($geo_zone_id_name, $options, set_value($geo_zone_id_name, $geo_zone_id), 'class="form-control" id="regions"');
                    ?>
                </div>
            </div>
        </div>
